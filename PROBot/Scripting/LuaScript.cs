...
namespace PROBot.Scripting
{
    public class LuaScript : BaseScript
    {
        ...
        private void CreateLuaInstance()
        {
            ...
            _lua.Globals["getActiveBattlers"] =
                new Func<Dictionary<string, Dictionary<string, int>>>(GetActiveBattlers);
            ...
            _lua.Globals["isTeamRangeSortedByLevelAscending"] =
                new Func<int, int, bool>(IsTeamRangeSortedByLevelAscending);
            _lua.Globals["isTeamRangeSortedByLevelDescending"] =
                new Func<int, int, bool>(IsTeamRangeSortedByLevelDescending);
            ...
            _lua.Globals["getPokemonRemainingExperienceFromPC"] =
                new Func<int, int, int>(GetPokemonRemainingExperienceFromPc);
            ...
            _lua.Globals["getPokemonRemainingPowerPointsFromPC"] =
                new Func<int, int, int, int>(GetPokemonRemainingPowerPointsFromPc);
            _lua.Globals["getPokemonMaxPowerPointsFromPC"] =
                new Func<int, int, int, int>(GetPokemonMaxPowerPointsFromPc);
            ...
            _lua.Globals["getPokemonMoveDamageTypeFromPC"] =
                new Func<int, int, int, string>(GetPokemonMoveDamageTypeFromPc);
            ...
            _lua.Globals["getPokemonIndividualValueFromPC"] =
                new Func<int, int, string, int>(GetPokemonIndividualValueFromPc);
            ...
            _lua.Globals["getPokemonOriginalTrainerFromPC"] =
                new Func<int, int, string>(GetPokemonOriginalTrainerFromPc);
        ...
        private void CallContent(string content)
        {
            try
            {
                TaskUtils.CallActionWithTimeout(() => _lua.DoString(content),
                    delegate { throw new Exception("The execution of the script timed out."); }, TimeoutDelay);
            }
        ...
        private Dictionary<string, Dictionary<string, int>> GetActiveBattlers()
        {
            ...
            foreach (var npc in Bot.Game.Map.Npcs.Where(npc => npc.CanBattle))
            {
        ...
        private List<Dictionary<string, int>> GetActiveDigSpots()
        {
            ...
            foreach (var npc in Bot.Game.Map.Npcs.Where(npc => npc.Type == 70 || npc.Type == 71))
            {
                var npcData = new Dictionary<string, int>();
                npcData["x"] = npc.PositionX;
                npcData["y"] = npc.PositionY;
...
*/
        ...
        private List<Dictionary<string, object>> GetSpawnList()
        {
            ...
            foreach (var pkmspwn in Bot.Game.SpawnList)
            {
        ...
        private List<Dictionary<string, int>> GetActiveHeadbuttTrees()
        {
            ...
            foreach (var npc in Bot.Game.Map.Npcs.Where(npc => npc.Type == 101))
            {
                var npcData = new Dictionary<string, int>();
                npcData["x"] = npc.PositionX;
                npcData["y"] = npc.PositionY;
        ...
        private List<Dictionary<string, int>> GetDiscoverableItems()
        {
            ...
            foreach (var npc in Bot.Game.Map.Npcs.Where(npc => npc.Type == 11 && npc.LosLength < 100))
            {
                var npcData = new Dictionary<string, int>();
                npcData["x"] = npc.PositionX;
                npcData["y"] = npc.PositionY;
        ...
        private int GetPokemonUniqueId(int pokemonUid)
        {
            ...
            var iv = Bot.Game.Team[pokemonUid - 1].Iv;
        ...
        private string GetPokemonMoveType(int index, int moveId)
        {
            ...
            var pokemon = Bot.Game.Team[index - 1];
            return pokemon.Moves[moveId - 1].Data.Type;
        }
        ...
        private double GetDamageMultiplier(string attacker, params DynValue[] defender)
        {
            ...
            {
                Fatal("error: getDamageMultiplier: the damage type '" + defender[0].CastToString() +
                      "' does not exist.");
                return -1;
            }
            ...
            {
                Fatal("error: getDamageMultiplier: the damage type '" + defender[1].CastToString() +
                      "' does not exist.");
                return -1;
            }
            ...
            power *= TypesManager.Instance.GetMultiplier(PokemonTypeExtensions.FromName(attacker),
                PokemonTypeExtensions.FromName(defender[0].CastToString()));
            power *= TypesManager.Instance.GetMultiplier(PokemonTypeExtensions.FromName(attacker),
                PokemonTypeExtensions.FromName(defender[1].CastToString()));
        ...
        private string GetPokemonHeldItem(int index)
        {
            ...
            var itemHeld = Bot.Game.Team[index - 1].ItemHeld;
        ...
        private int GetRemainingPowerPoints(int pokemonIndex, string moveName)
        {
            ...
            var move = Bot.Game.Team[pokemonIndex - 1].Moves.FirstOrDefault(m =>
                MovesManager.Instance.GetMoveData(m.Id)?.Name.ToUpperInvariant() == moveName);
            if (move == null)
            {
                Fatal("error: getRemainingPowerPoints: the pokémon " + pokemonIndex + " does not have a move called '" +
                      moveName + "'.");
                return 0;
            }
        ...
        private static readonly Dictionary<string, StatType> _stats = new Dictionary<string, StatType>
        {
            {"HP", StatType.Health},
        ...
        private bool IsOpponentEffortValue(string statType)
        {
            ...
            var stats = EffortValuesManager.Instance.BattleValues[Bot.Game.ActiveBattle.OpponentId];
        ...
        {
            var alreadyInCell = cellTypePredicate(Bot.Game.PlayerX, Bot.Game.PlayerY);
        ...
        private bool MoveNearExit(string mapName)
        {
            ...
            var nearest = Bot.Game.Map
                .GetNearestLinks(mapName.ToUpperInvariant(), Bot.Game.PlayerX, Bot.Game.PlayerY).First();
            if (nearest == null)
            {
            ...
            do
            {
                ...
                if (++tries > 100) return false;
            } while (x <= 0 || y <= 0 || !Bot.Game.Map.IsNormalGround(x, y) ||
                     x == Bot.Game.PlayerX && y == Bot.Game.PlayerY);
        ...
        private bool TalkToNpc(string npcName)
        {
            ...
            npcName = npcName.ToUpperInvariant();
            var target = Bot.Game.Map.Npcs.FirstOrDefault(npc => npc.Name.ToUpperInvariant() == npcName);
            if (target == null)
            {
        ...
        private bool TalkToNpcOnCell(int cellX, int cellY)
        {
            ...
            var target = Bot.Game.Map.Npcs.FirstOrDefault(npc => npc.PositionX == cellX && npc.PositionY == cellY);
            if (target == null)
            {
        ...
        private bool UsePokecenter()
        {
            ...
            var nurse = Bot.Game.Map.Npcs.FirstOrDefault(npc => npc.Name.StartsWith("Nurse"));
            if (nurse == null)
            {
                Fatal("error: usePokecenter: could not find the Nurse Joy.");
                return false;
            }
            var target = Bot.Game.Map.Npcs.FirstOrDefault(npc =>
                npc.PositionX == nurse.PositionX && npc.PositionY == nurse.PositionY + 1);
            if (target == null)
            {
        ...
        private bool SwapPokemonWithLeader(string pokemonName)
        {
            ...
            var pokemon = Bot.Game.FindFirstPokemonInTeam(pokemonName.ToUpperInvariant());
            if (pokemon == null)
            {
        ...
        {
            var dt = Convert.ToDateTime(Bot.Game.PokemonTime);
            minute = dt.Minute;
            return dt.Hour;
        }
        ...
        private bool IsPcAccessValid(string functionName, int boxId, int boxPokemonId)
        {
            ...
            {
                Fatal("error: " + functionName + ": tried to access box #" + boxId +
                      " different from the currently loaded box.");
                return false;
            }
            if (boxPokemonId < 1 || boxPokemonId > Bot.Game.CurrentPcBox.Count)
            {
                Fatal("error: " + functionName + ": tried to access the unknown pokemon #" + boxPokemonId +
                      " of the box #" + boxId + ".");
                return false;
            }
            return true;
        ...
        private int GetPokemonUniqueIdFromPc(int boxId, int boxPokemonId)
        {
            if (!IsPcAccessValid("getPokemonUniqueIdFromPC", boxId, boxPokemonId))
                return -1;
            var iv = Bot.Game.CurrentPcBox[boxPokemonId - 1].Iv;
        ...
        private int GetPokemonHealthPercentFromPc(int boxId, int boxPokemonId)
        {
            if (!IsPcAccessValid("getPokemonCurrentHealthPercentFromPC", boxId, boxPokemonId))
                return -1;
            var pokemon = Bot.Game.CurrentPcBox[boxPokemonId - 1];
            return pokemon.CurrentHealth * 100 / pokemon.MaxHealth;
        }
        ...
        private string GetPokemonMoveTypeFromPc(int boxId, int boxPokemonId, int moveId)
        {
            ...
            return Bot.Game.CurrentPcBox[boxPokemonId - 1].Moves[moveId - 1].Data.Type;
        }
        ...
        private string GetPokemonHeldItemFromPc(int boxId, int boxPokemonId)
        {
            if (!IsPcAccessValid("getPokemonHeldItemFromPC", boxId, boxPokemonId))
                return null;
            var itemHeld = Bot.Game.CurrentPcBox[boxPokemonId - 1].ItemHeld;
            return itemHeld == string.Empty ? null : itemHeld;
        }
        ...
        private bool ReleasePokemonFromTeam(int pokemonUid)
        {
            if (pokemonUid < 1 || pokemonUid > 6 || pokemonUid > Bot.Game.Team.Count)
            {
                Fatal("error: releasePokemonFromTeam: pokemonUid is out of range: " + pokemonUid
                      + " (team size: " + Bot.Game.Team.Count + ").");
                return false;
            }
            if (!Bot.Game.IsPcOpen)
            {
                Fatal("error: releasePokemonFromTeam: cannot release a pokemon while the PC is closed: #" + pokemonUid +
                      " (" + Bot.Game.Team[pokemonUid].Name + ").");
                return false;
            }
            if (Bot.Game.IsPcBoxRefreshing)
            {
                Fatal("error: releasePokemonFromTeam: cannot release a pokemon while the PC box is refreshing: #" +
                      pokemonUid + " (" + Bot.Game.Team[pokemonUid].Name + ").");
                return false;
            }
        ...
        private bool BuyItem(string itemName, int quantity)
        {
            ...
            var item = Bot.Game.OpenedShop.Items.FirstOrDefault(i =>
        ...
        private bool ForgetMove(string moveName)
        {
            ...
            moveName = moveName.ToUpperInvariant();
            var pokemon = Bot.Game.Team[Bot.MoveTeacher.PokemonUid - 1];
            var move = pokemon.Moves.FirstOrDefault(m =>
                MovesManager.Instance.GetMoveData(m.Id)?.Name.ToUpperInvariant() == moveName);
        ...
        private bool ForgetAnyMoveExcept(DynValue[] moveNames)
        {
            ...
            foreach (var value in moveNames)
                movesInvariantNames.Add(value.CastToString().ToUpperInvariant());

            var pokemon = Bot.Game.Team[Bot.MoveTeacher.PokemonUid - 1];
            var move = pokemon.Moves.FirstOrDefault(m =>
                !movesInvariantNames.Contains(MovesManager.Instance.GetMoveData(m.Id)?.Name.ToUpperInvariant()));
        ...
        {
            var input = new InputDialog.InputDialog("Input", "Please select:", options);
            var selection = "error";
        ...
        private void Login(string accountName, string password, string server, int socks = 0, string host = "",
            int port = 0, string socksUser = "", string socksPass = "")
        {
        ...
        private void Relog(float seconds, string message)
        {
            ...
            var server = DynValue.NewString(Bot.Account.Server);
            ...
            if (Bot.Account.Socks.Version != SocksVersion.None)
            {
                ...
                Invoke(_lua.Globals.Get("login"), seconds, name, password, server, socks, host, port, socksUser,
                    socksPass);
            }
            else
            {
